...
# GPU 3 reserved for reward update (set globally in run-full-pipeline-job.sh)
...
if [ -n "${ACTOR_CKPT:-}" ] && [[ "$ACTOR_CKPT" != "/path/to/"* ]]; then
  CKPT_ARGS+=(--load ${ACTOR_CKPT})
fi
...
IRL_ARGS=(
   ...
   --reward-eval-path ${SLIME_ROOT}/hh-rlhf-processed/hh-rlhf-merged-test.jsonl
   --reward-eval-prompt-key text
   --reward-eval-chosen-key chosen
   --reward-eval-rejected-key rejected
   --reward-eval-max-samples 200
   --reward-model-dir ${SLIME_ROOT}/models/reward_model
   --reward-update-interval 3
   --reward-update-epochs 1
   --reward-update-batch-size 32
   --reward-update-lr 5e-5
   ...
   --win-rate-eval-path ${SLIME_ROOT}/hh-rlhf-processed/hh-rlhf-merged-test.jsonl
