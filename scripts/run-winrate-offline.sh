...
WINRATE_REFERENCE_DATA=${WINRATE_REFERENCE_DATA:-$ULTRAFEEDBACK_DIR/uf-test.jsonl}
WINRATE_REFERENCE_KEY=${WINRATE_REFERENCE_KEY:-chosen}
WINRATE_REFERENCE_PROMPT_KEY=${WINRATE_REFERENCE_PROMPT_KEY:-text}
WINRATE_ENABLE_REFERENCE=${WINRATE_ENABLE_REFERENCE:-1}
...
scan_once() {
  ...
  if [ "$WINRATE_ENABLE_REFERENCE" = "1" ] && [ -f "$WINRATE_REFERENCE_DATA" ]; then
  ...
  for round in "${ROUNDS[@]}"; do
    ...
    if [ "$WINRATE_ENABLE_REFERENCE" != "1" ]; then
...
echo "WINRATE_ENABLE_REFERENCE=$WINRATE_ENABLE_REFERENCE"
